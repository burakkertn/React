...
const options = {
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: 'İngiliz Sterlini Chart Gösterimi',
    },
  },
};
...
const Horizontalchart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const url = 'https://localhost:44317/api/Doviz/gbp';
      ...
      try {
        ...
        const usdData = apiData.map(item => item.gbp);
        ...
        // Örnek olarak:
        const lineUrl = 'https://localhost:44317/api/Doviz/gbp';
        ...
        const lineDataPoints = lineApiData.map(item => item.gbp);
