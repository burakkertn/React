...
const options = {
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: 'Japon Yeni Chart Gösterimi',
    },
  },
};
...
const Horizontalchart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const url = 'https://localhost:44317/api/Doviz/jpy';
      ...
      try {
        ...
        const usdData = apiData.map(item => item.jpy);
        ...
        // Örnek olarak:
        const lineUrl = 'https://localhost:44317/api/Doviz/jpy';
        ...
        const lineDataPoints = lineApiData.map(item => item.jpy);
